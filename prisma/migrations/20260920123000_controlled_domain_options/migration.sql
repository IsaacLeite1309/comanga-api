-- Entrega 4 — classificações controladas do catálogo (seções 11 a 14 do guia).
...
BEGIN
    ...
    LOOP
        ...
        ORDER BY (code IS NOT DISTINCT FROM official.option_code) DESC, id ASC
        LIMIT 1;
        ...
        IF matched_id IS NULL THEN
            ...
            RAISE NOTICE 'Entrega 4: tipo de Obra oficial criado: %.', official.option_label;
        ELSE
            UPDATE domain_option_values
            SET code = official.option_code,
                system_managed = TRUE,
...
BEGIN
    ...
    LOOP
        ...
        IF matched_id IS NULL THEN
            INSERT INTO domain_option_values (category_id, label, code, system_managed, "position", active)
            VALUES (category_id_value, official.option_label, official.option_code, TRUE, official.option_position, TRUE);
            RAISE NOTICE 'Entrega 4: gênero oficial criado: %.', official.option_label;
        ELSE
            UPDATE domain_option_values
            SET code = official.option_code,
                system_managed = TRUE,
...
BEGIN
    ...
    LOOP
        RAISE NOTICE 'Entrega 4: dependência não oficial removida do tipo "%" com o país "%".',
...
-- Tipos de Edição recebem posições 0..n-1 pela ordem alfabética atual, uma única vez:
-- se alguma posição já foi definida por um administrador, a ordenação manual é preservada.
UPDATE domain_option_values AS target
SET "position" = ordered.new_position
FROM (
    SELECT
        value.id,
        (ROW_NUMBER() OVER (ORDER BY value.label ASC, value.id ASC) - 1)::INTEGER AS new_position
    FROM domain_option_values value
    JOIN domain_option_categories category
        ON category.id = value.category_id AND category.slug = 'tipos-edicao'
) AS ordered
WHERE target.id = ordered.id
  AND target."position" <> ordered.new_position
  AND NOT EXISTS (
      SELECT 1
      FROM domain_option_values existing
      JOIN domain_option_categories category
          ON category.id = existing.category_id AND category.slug = 'tipos-edicao'
      WHERE existing."position" <> 0
  );
...
BEGIN
    ...
    IF normalized > 0 THEN
        RAISE NOTICE 'Entrega 4: % Obra(s) com Hentai tiveram adult_content normalizado para TRUE.', normalized;
    END IF;
END
$normalize_adult$;
...
BEGIN
    ...
    LOOP
        RAISE NOTICE 'Entrega 4: valor sem correspondência oficial mantido intacto — categoria %, id %, rótulo "%". O responsável decidirá seu destino.',
